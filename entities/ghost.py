...
#     #         if self.is_valid_move(new_x, new_y):  # Kiểm tra nếu bước đi hợp lệ
...
#         if self.path:
#             next_pos = self.path.pop(0)
#             # print(f"Ghost {self.color} di chuyển từ ({self.x}, {self.y}) đến {next_pos}")  # Debug
#             new_x, new_y = next_pos

#             # if self.is_valid_move(new_x, new_y):  # Kiểm tra nếu bước đi hợp lệ
#             self.x, self.y = new_x, new_y
#             sleep(0.1)  # Thời gian nghỉ giữa các bước di chuyển
#             self.update_direction(new_x, new_y)  # Cập nhật hướng di chuyển
...
#         # Render ghost on the screen at the correct position
