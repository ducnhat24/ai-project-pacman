...
class MazeDrawing:
    def __init__(self, screen):
        self.map = copy.deepcopy(Board.maze)  # Initialize the maze from Board
        self.screen = screen

    title_width = Config.TITLE_WIDTH
    ...
    def draw(self):
        ...
        total_map_width_px = num_cols * self.title_width
        total_map_height_px = num_rows * self.title_height
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                draw_base_x = j * self.title_width + offset_x
                draw_base_y = i * self.title_height + offset_y

                # Tọa độ tâm của ô hiện tại (đã cộng offset)
                draw_center_x = draw_base_x + (self.title_width / 2)
                draw_center_y = draw_base_y + (self.title_height / 2)
                ...
                int_title_width = int(self.title_width)
                int_title_height = int(self.title_height)
                ...
                if cell == 1: # normal food
                    ...
                    pygame.draw.line(self.screen, Color.color_wall,
                                    ...
                                    (int_center_x, int_base_y + int_title_height), # Điểm kết thúc (dưới)
                                    4)

                elif cell == 4: # horizontal wall (vẽ đường ngang giữa ô)
                    pygame.draw.line(self.screen, Color.color_wall,
                                    (int_base_x, int_center_y), # Điểm bắt đầu (trái)
                                    (int_base_x + int_title_width, int_center_y), # Điểm kết thúc (phải)
                                    4)
                ...
                # Dưới đây là áp dụng offset vào các tính toán gốc đó, nhưng bạn có thể cần xem lại logic vẽ arc.
                    ...
                    # Tính toán gốc: [(j * self.title_width - (self.title_width * 0.4)) - 2, cy, self.title_width, self.title_height]
                    # Áp dụng offset:
                    rect_left = int(draw_base_x - int_title_width * 0.4 - 2) # Tọa độ left của rect
                    rect_top = int(draw_center_y) # Tọa độ top của rect (dựa trên cy gốc)
                    arc_rect = pygame.Rect(rect_left, rect_top, int_title_width, int_title_height)
                    try:
                    ...
                    # Tính toán gốc: [(j * self.title_width + (self.title_width * 0.5)), cy, self.title_width, self.title_height]
                    # Áp dụng offset:
                    rect_left = int(draw_base_x + int_title_width * 0.5)
                    ...
                    # Tính toán gốc: [(j * self.title_width + (self.title_width * 0.5)), (i * self.title_height - (0.4 * self.title_height)), self.title_width, self.title_height]
                    # Áp dụng offset:
                    rect_left = int(draw_base_x + int_title_width * 0.5)
                    rect_top = int(draw_base_y - int_title_height * 0.4)
                    arc_rect = pygame.Rect(rect_left, rect_top, int_title_width, int_title_height)
                    try:
                    ...
                    # Tính toán gốc: [(j * self.title_width - (self.title_width * 0.4)) - 2, (i * self.title_height - (0.4 * self.title_height)), self.title_width, self.title_height]
                    # Áp dụng offset:
                    rect_left = int(draw_base_x - int_title_width * 0.4 - 2)
                    rect_top = int(draw_base_y - int_title_height * 0.4)
                    arc_rect = pygame.Rect(rect_left, rect_top, int_title_width, int_title_height)
                    try:
                    ...
                    pygame.draw.line(self.screen, Color.color_fence,
                                    ...
                                    (int_base_x + int_title_width, int_center_y),
                                    4)
